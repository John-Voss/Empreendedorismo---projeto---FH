...
var pergunta1, p1_image, p2_image, p3_image, p4_image;
var sim, nao;
...
function setup(){
  ...
  solo_invis = createSprite(width - 1400, height - 15, 800, 10);
  solo_invis.visible = false;
  jogador = createSprite(width - 1500, height - 75, 35, 80);
  ...
  sim = createSprite(width - 800, height - 250, 50, 50);
  nao = createSprite(width - 850, height - 250, 50, 50);
...
function draw(){
  ...
  fill('black');
  textSize(20);
  text('R$ '+ pontuacao, width - 1500, height - 700);
}
function money() {
  if(frameCount% 200 === 0) {
...
function moneyDie() {
  if(frameCount% 50 === 0) {
    dinheiro$ = createSprite(width - width, Math.round(random(height - 700, height - 250), 20, 20));
